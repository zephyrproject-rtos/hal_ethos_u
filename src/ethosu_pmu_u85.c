...
void U85_PMU_Enable(struct ethosu_driver *drv)
{
...
void U85_PMU_Disable(struct ethosu_driver *drv)
{
...
uint32_t U85_PMU_Get_NumEventCounters(void)
{
...
void U85_PMU_CYCCNT_Reset(struct ethosu_driver *drv)
{
...
void U85_PMU_EVCNTR_ALL_Reset(struct ethosu_driver *drv)
{
...
void U85_PMU_CNTR_Enable(struct ethosu_driver *drv, uint32_t mask)
{
    LOG_DEBUG("mask=0x%08" PRIx32, mask);
    drv->dev.reg->PMCNTENSET.word = mask;
}

void U85_PMU_CNTR_Disable(struct ethosu_driver *drv, uint32_t mask)
{
    LOG_DEBUG("mask=0x%08" PRIx32, mask);
    drv->dev.reg->PMCNTENCLR.word = mask;
}

uint32_t U85_PMU_CNTR_Status(struct ethosu_driver *drv)
{
...
uint64_t U85_PMU_Get_CCNTR(struct ethosu_driver *drv)
{
...
void U85_PMU_Set_CCNTR(struct ethosu_driver *drv, uint64_t val)
{
    uint32_t active = U85_PMU_CNTR_Status(drv) & ETHOSU_PMU_CCNT_Msk;

    LOG_DEBUG("val=%" PRIu64, val);

    if (active)
    {
        U85_PMU_CNTR_Disable(drv, ETHOSU_PMU_CCNT_Msk);
    }
    ...
    if (active)
    {
        U85_PMU_CNTR_Enable(drv, ETHOSU_PMU_CCNT_Msk);
    }
}
...
uint32_t U85_PMU_Get_CNTR_OVS(struct ethosu_driver *drv)
{
    LOG_DEBUG("");
    return drv->dev.reg->PMOVSSET.word;
}

void U85_PMU_Set_CNTR_OVS(struct ethosu_driver *drv, uint32_t mask)
{
    LOG_DEBUG("");
    drv->dev.reg->PMOVSCLR.word = mask;
}

void U85_PMU_Set_CNTR_IRQ_Enable(struct ethosu_driver *drv, uint32_t mask)
{
    LOG_DEBUG("mask=0x%08" PRIx32, mask);
    drv->dev.reg->PMINTSET.word = mask;
}

void U85_PMU_Set_CNTR_IRQ_Disable(struct ethosu_driver *drv, uint32_t mask)
{
    LOG_DEBUG("mask=0x%08" PRIx32, mask);
    drv->dev.reg->PMINTCLR.word = mask;
}

uint32_t U85_PMU_Get_IRQ_Enable(struct ethosu_driver *drv)
{
...
void U85_PMU_CNTR_Increment(struct ethosu_driver *drv, uint32_t mask)
{
    LOG_DEBUG("");
    uint32_t cntrs_active = U85_PMU_CNTR_Status(drv);

    // Disable counters
    U85_PMU_CNTR_Disable(drv, mask);

    // Increment cycle counter
    if (mask & ETHOSU_PMU_CCNT_Msk)
    {
        uint64_t val                       = U85_PMU_Get_CCNTR(drv) + 1;
        drv->dev.reg->PMCCNTR.CYCLE_CNT_LO = val & MASK_0_31_BITS;
        drv->dev.reg->PMCCNTR.CYCLE_CNT_HI = (val & MASK_32_47_BITS) >> 32;
    }
    ...
    for (int i = 0; i < NPU_REG_PMEVCNTR_ARRLEN; i++)
    {
        if (mask & (1u << i))
        {
            uint32_t val                   = U85_PMU_Get_EVCNTR(drv, i);
            drv->dev.reg->PMEVCNTR[i].word = val + 1;
        }
    }

    // Reenable the active counters
    U85_PMU_CNTR_Enable(drv, cntrs_active);
}

void U85_PMU_PMCCNTR_CFG_Set_Start_Event(struct ethosu_driver *drv, enum ethosu_pmu_event_type start_event)
{
...
void U85_PMU_PMCCNTR_CFG_Set_Stop_Event(struct ethosu_driver *drv, enum ethosu_pmu_event_type stop_event)
{
...
uint32_t U85_PMU_Get_QREAD(struct ethosu_driver *drv)
{
...
uint32_t U85_PMU_Get_STATUS(struct ethosu_driver *drv)
{
...
static const struct ethosu_pmu_ops u85_ops = {
    .ETHOSU_PMU_Enable                      = U85_PMU_Enable,
    .ETHOSU_PMU_Disable                     = U85_PMU_Disable,
    .ETHOSU_PMU_Get_NumEventCounters        = U85_PMU_Get_NumEventCounters,
    .ETHOSU_PMU_Set_EVTYPER                 = U85_PMU_Set_EVTYPER,
    .ETHOSU_PMU_Get_EVTYPER                 = U85_PMU_Get_EVTYPER,
    .ETHOSU_PMU_CYCCNT_Reset                = U85_PMU_CYCCNT_Reset,
    .ETHOSU_PMU_EVCNTR_ALL_Reset            = U85_PMU_EVCNTR_ALL_Reset,
    .ETHOSU_PMU_CNTR_Enable                 = U85_PMU_CNTR_Enable,
    .ETHOSU_PMU_CNTR_Disable                = U85_PMU_CNTR_Disable,
    .ETHOSU_PMU_CNTR_Status                 = U85_PMU_CNTR_Status,
    .ETHOSU_PMU_Get_CCNTR                   = U85_PMU_Get_CCNTR,
    .ETHOSU_PMU_Set_CCNTR                   = U85_PMU_Set_CCNTR,
    .ETHOSU_PMU_Get_EVCNTR                  = U85_PMU_Get_EVCNTR,
    .ETHOSU_PMU_Set_EVCNTR                  = U85_PMU_Set_EVCNTR,
    .ETHOSU_PMU_Get_CNTR_OVS                = U85_PMU_Get_CNTR_OVS,
    .ETHOSU_PMU_Set_CNTR_OVS                = U85_PMU_Set_CNTR_OVS,
    .ETHOSU_PMU_Set_CNTR_IRQ_Enable         = U85_PMU_Set_CNTR_IRQ_Enable,
    .ETHOSU_PMU_Set_CNTR_IRQ_Disable        = U85_PMU_Set_CNTR_IRQ_Disable,
    .ETHOSU_PMU_Get_IRQ_Enable              = U85_PMU_Get_IRQ_Enable,
    .ETHOSU_PMU_CNTR_Increment              = U85_PMU_CNTR_Increment,
    .ETHOSU_PMU_PMCCNTR_CFG_Set_Start_Event = U85_PMU_PMCCNTR_CFG_Set_Start_Event,
    .ETHOSU_PMU_PMCCNTR_CFG_Set_Stop_Event  = U85_PMU_PMCCNTR_CFG_Set_Stop_Event,
    .ETHOSU_PMU_Get_QREAD                   = U85_PMU_Get_QREAD,
    .ETHOSU_PMU_Get_STATUS                  = U85_PMU_Get_STATUS,
};

const struct ethosu_pmu_desc ethosu_pmu_desc_u85 = {
    .ops = &u85_ops,
};
